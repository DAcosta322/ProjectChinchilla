...
POSITION_LIMITS = {
    ...
    # ROUND 5 — all 50 products at limit 10
    ...
    "PEBBLES_XS": 10, "PEBBLES_S": 10, "PEBBLES_M": 10, "PEBBLES_L": 10, "PEBBLES_XL": 10,
    ...
    "PANEL_1X2": 10, "PANEL_2X2": 10, "PANEL_1X4": 10, "PANEL_2X4": 10, "PANEL_4X4": 10,
    "OXYGEN_SHAKE_MORNING_BREATH": 10, "OXYGEN_SHAKE_EVENING_BREATH": 10,
    "OXYGEN_SHAKE_MINT": 10, "OXYGEN_SHAKE_CHOCOLATE": 10, "OXYGEN_SHAKE_GARLIC": 10,
    "SNACKPACK_CHOCOLATE": 10, "SNACKPACK_VANILLA": 10, "SNACKPACK_PISTACHIO": 10,
    "SNACKPACK_STRAWBERRY": 10, "SNACKPACK_RASPBERRY": 10,
}
...
def run_backtest(
    ...
    # Build per-timestamp total PnL series (sum across all products incl. MTM)
    # for fragment-level robustness analysis. Cheap to compute from activity_rows.
    pnl_at_ts: Dict[int, float] = {}
    pnl_by_prod_at_ts: Dict[str, Dict[int, float]] = {p: {} for p in products}
    for row in activity_rows[1:]:  # skip header
        parts = row.split(";")
        ts_i = int(parts[1]); prod = parts[2]; pp = float(parts[16])
        pnl_at_ts[ts_i] = pnl_at_ts.get(ts_i, 0.0) + pp
        pnl_by_prod_at_ts[prod][ts_i] = pp
    ...
    return {
        ...
        "pnl_at_ts": pnl_at_ts,
        "pnl_by_prod_at_ts": pnl_by_prod_at_ts,
    }
