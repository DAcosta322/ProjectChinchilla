"""R11: Cap aggressive takes at fv (not fv_eff).

Live-fill analysis on submission 416716 showed HYDROGEL aggressive
buys paid ~+4.87 ticks slippage vs the best ask available within
neighbouring 2000-ts windows. Root cause: when target is at +200
and pos=0, fv_eff = fv + 15, so the take loop accepts asks above
the mid - "paying up" for size. This burns spread on every entry.

Fix: require take price <= fv (the actual mid), not fv_eff. Aggressive
takes only when the market is offering BELOW our true valuation. The
INV_SKEW shift is still applied to posted quotes so we keep building
inventory passively, just not paying premium.

Other features unchanged from final.py.
"""
...
class Trader:
    ...
    def _do_orders(self, P, fv, pos, target_pos, od):
        ...
        fv_eff = fv - skew
        # NEW: cap aggressive-take threshold at fv (not fv_eff)
        # We never pay above mid for a buy or sell below mid for a sell.
        take_buy_th = min(fv_eff, fv) if P.TAKE_AT_FAIR else fv_eff
        take_sell_th = max(fv_eff, fv) if P.TAKE_AT_FAIR else fv_eff
        best_bid = max(od.buy_orders.keys()); best_ask = min(od.sell_orders.keys())
        buy_cap = P.POS_LIMIT - pos; sell_cap = P.POS_LIMIT + pos
        for price in sorted(od.sell_orders.keys()):
            if price < take_buy_th and buy_cap > 0:
                qty = min(-od.sell_orders[price], buy_cap)
                orders.append(Order(P.SYMBOL, price, qty)); buy_cap -= qty
        for price in sorted(od.buy_orders.keys(), reverse=True):
            if price > take_sell_th and sell_cap > 0:
