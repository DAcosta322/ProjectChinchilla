"""R2: Trade-tape (aggressor flow) signal.

Watch market_trades from previous tick. Classify aggressor by trade
price vs mid: price > mid -> buyer lifted (positive flow). Sum
signed qty = net flow. MR interpretation: lots of recent buy flow ->
price ran up -> expect revert -> short bias.

  target += -FLOW_GAIN * recent_net_flow_normalized
"""
...
class HydrogelParams:
    ...
    PROFIT_DIST = 40
    PROFIT_RANGE = 40
    FLOW_GAIN = 5           # target += -FLOW_GAIN * normalized_flow
...
def _net_flow(market_trades, mid: float) -> float:
    """Signed flow from last-tick market trades. + = net buying."""
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if HydrogelParams.SYMBOL in state.order_depths:
            ...
            if fv is not None:
                ...
                _, orders = self._mr(state, HydrogelParams, fv, ema, avg_entry, mt)
        ...
        if VelvetParams.SYMBOL in state.order_depths:
            ...
            if velvet_mid is not None:
                ...
                tp, orders = self._mr(state, VelvetParams, velvet_mid, ema, avg_entry, mt)
    ...
    def _mr(self, state, P, fv, ema, avg_entry, market_trades):
        ...
        if P.ANCHOR_SPAN > 0:
            ...
            if P.FLOW_GAIN > 0:
                flow = _net_flow(market_trades, fv)
                raw += -P.FLOW_GAIN * flow
