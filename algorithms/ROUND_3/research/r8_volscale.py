"""R8: Vol-adaptive MR_STRENGTH on top of eff4.

Track rolling realized vol (EWMA std of returns). Scale MR_STRENGTH:
  vol_factor = clip(VOL_REF / max(vol, 1), 0.5, 2.0)
  effective MR_STRENGTH = MR_STRENGTH * vol_factor

When vol is HIGH (price moves big), reduce MR aggression (don't
build huge positions on noisy moves). When vol is LOW (tight chop),
increase MR aggression (each tick is more reliably a small dip).
"""
...
class HydrogelParams:
    ...
    VOL_SPAN = 200; VOL_REF = 2.0     # ref std of 1-tick returns
...
class VelvetParams:
    ...
    VOL_SPAN = 0; VOL_REF = 1.0   # disabled - hurts VELVET
    ...
    def _mr(self, state, P, fv, ema, avg_entry, prev_fv, var_ewma):
        ...
        if P.VOL_SPAN > 0:
            if P.SYMBOL in prev_fv:
                ...
                prev_var = var_ewma.get(P.SYMBOL, 1.0)
                a = 2.0 / (P.VOL_SPAN + 1.0)
                var_new = a * (ret * ret) + (1 - a) * prev_var
                var_ewma[P.SYMBOL] = var_new
            prev_fv[P.SYMBOL] = fv
        ...
        if P.ANCHOR_SPAN > 0:
            ...
            ema[P.SYMBOL] = anchor
            mr_strength_eff = P.MR_STRENGTH * vol_factor
            target = max(-P.POS_LIMIT, min(P.POS_LIMIT,
                int(round(-mr_strength_eff * (fv - anchor)))))
