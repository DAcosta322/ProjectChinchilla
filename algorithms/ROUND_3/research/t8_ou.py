"""T8 #8: OU explicit MR with optimal threshold + half-life.

Fit dX = theta*(mu - X)*dt + sigma*dW. Online estimates of theta, mu,
sigma via running covariance regression of dX on (mu - X).

Optimal entry threshold (Cartea/Jaimungal style for symmetric OU):
  z* = sigma / sqrt(2 * theta) * c   (c is risk-tuned multiplier)
  Enter long when X < mu - z*, short when X > mu + z*.

Use: when |X - mu| > k * std_OU, scale target up; otherwise standard MR.
Built on top of OFI-enhanced final.
"""
...
class HydrogelParams:
    ...
    OU_BOOST = 1.5          # extra MR_STRENGTH multiplier when |fv-anchor| > z*
    OU_Z_TH = 1.5           # z-threshold (in sigma_OU units)
...
class Trader:
    ...
    def run(self, state):
        ...
        # OU sigma estimate (EWMA of (fv - anchor)^2) gives sigma_OU
        ou_var = td.get("ouv", {})
        ...
        if HydrogelParams.SYMBOL in state.order_depths:
            ...
            if fv is not None:
                _, orders = self._mr(state, HydrogelParams, fv, ema, avg_entry,
                                     prev_fv, var_ewma, prev_book, ofi_ewma, ou_var)
        ...
        return result, 0, json.dumps({"ema": ema, "avg": avg_entry, "tv": tv, "tvn": tv_n,
                                      ...
                                      "pbook": prev_book, "ofi": ofi_ewma, "ouv": ou_var})

    def _mr(self, state, P, fv, ema, avg_entry, prev_fv, var_ewma, prev_book, ofi_ewma, ou_var):
        ...
        if P.ANCHOR_SPAN > 0:
            ...
            ema[P.SYMBOL] = anchor
            # OU sigma: EWMA of (fv-anchor)^2
            dev = fv - anchor
            prev_ouv = ou_var.get(P.SYMBOL, P.VOL_REF * P.VOL_REF)
            ou_var[P.SYMBOL] = 0.01 * dev * dev + 0.99 * prev_ouv
            sigma_ou = math.sqrt(max(0.5, ou_var[P.SYMBOL]))

            mr_eff = P.MR_STRENGTH * vol_factor
            # OU boost: if |dev| > Z_TH * sigma_ou, increase MR_STRENGTH
            if abs(dev) > P.OU_Z_TH * sigma_ou:
                mr_eff *= P.OU_BOOST
            raw = -mr_eff * dev + P.OFI_GAIN * ofi_smooth
