"""T2 #2: Cont-Stoikov Order Flow Imbalance.

OFI uses CHANGES in level-1 volumes as a leading price indicator:
  e_n = I(P_b_n >= P_b_n-1) * q_b_n - I(P_b_n <= P_b_n-1) * q_b_n-1
      - I(P_a_n <= P_a_n-1) * q_a_n + I(P_a_n >= P_a_n-1) * q_a_n-1
  expected price change ~ lambda * OFI

Add OFI-derived signal to MR target.
"""
...
class HydrogelParams:
    ...
    OFI_GAIN = 1.0       # target += OFI_GAIN * smoothed_OFI
    OFI_SPAN = 20        # smoothing of OFI
...
def _ofi_step(prev, cur):
    """Cont-Stoikov OFI for one tick. prev, cur = (bb, bv, ba, av)."""
...
class Trader:
        ...
        if HydrogelParams.SYMBOL in state.order_depths:
            ...
            if fv is not None:
                _, orders = self._mr(state, HydrogelParams, fv, ema, avg_entry,
                                     prev_fv, var_ewma, prev_book, ofi_ewma)
    ...
    def _mr(self, state, P, fv, ema, avg_entry, prev_fv, var_ewma, prev_book, ofi_ewma):
        ...
        if P.ANCHOR_SPAN > 0:
            ...
            # Add OFI bias - positive OFI suggests upward pressure -> long
            raw += P.OFI_GAIN * ofi_smooth
