"""R6: Layered passive quoting.

Instead of single bid+ask, post N_LAYERS at progressively worse prices
spaced LAYER_STEP ticks apart. Captures partial-fill flow at multiple
depths. Total quoted size split across layers.

  layer i price: our_bid - i*LAYER_STEP, qty = total/N_LAYERS
"""
...
class HydrogelParams:
    SYMBOL = "HYDROGEL_PACK"; POS_LIMIT = 200
    INV_SKEW = 15; ANCHOR_SPAN = 500; MR_STRENGTH = 5
    PROFIT_DIST = 40; PROFIT_RANGE = 40
    N_LAYERS = 3; LAYER_STEP = 4


class VelvetParams:
    SYMBOL = "VELVETFRUIT_EXTRACT"; POS_LIMIT = 200
    INV_SKEW = 3; ANCHOR_SPAN = 5000; MR_STRENGTH = 10
    PROFIT_DIST = 15; PROFIT_RANGE = 15
    N_LAYERS = 2; LAYER_STEP = 1
...
class Trader:
    ...
    def _do_orders(self, P, fv, pos, target_pos, od):
        ...
        n_layers = max(1, getattr(P, 'N_LAYERS', 1))
        step = max(0, getattr(P, 'LAYER_STEP', 0))
        base_bid = best_bid + 1; base_ask = best_ask - 1
        if base_bid >= base_ask: base_bid = best_bid; base_ask = best_ask
        shift = int(round(-skew))
        our_bid_top = min(base_bid + shift, int(fv_eff))
        our_ask_top = max(base_ask + shift, int(fv_eff) + 1)
        if our_ask_top <= our_bid_top: our_ask_top = our_bid_top + 1

        if buy_cap > 0:
            per = max(1, buy_cap // n_layers)
            remaining = buy_cap
            for i in range(n_layers):
                if remaining <= 0: break
                price = our_bid_top - i * step
                qty = min(per, remaining) if i < n_layers - 1 else remaining
                orders.append(Order(P.SYMBOL, price, qty))
                remaining -= qty
        if sell_cap > 0:
            per = max(1, sell_cap // n_layers)
            remaining = sell_cap
            for i in range(n_layers):
                if remaining <= 0: break
                price = our_ask_top + i * step
                qty = min(per, remaining) if i < n_layers - 1 else remaining
                orders.append(Order(P.SYMBOL, price, -qty))
                remaining -= qty
        return orders
