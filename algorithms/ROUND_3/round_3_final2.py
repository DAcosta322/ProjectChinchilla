"""Round 3 FINAL v2 - adds HYD range-position blend on top of round_3_final.

Background: submission 438658 (round_3_final) lost ~$1.4K on HYD vs the
simpler "smooth range-position" submission 388019 because its EMA(500)
anchor catches up to drifting price within a day, collapsing the MR
target during the bounce. 388019's rolling [lo, hi] window stays sticky
and pushes target through zero into a real short on the rebound.

This version keeps everything from round_3_final and ADDS to HYD's
target a range-position component:
    target += RANGE_GAIN * (midpoint - fv)
where midpoint = (rolling_lo + rolling_hi) / 2 over WINDOW recent fvs.
Only active once the window is full and rng >= MIN_RANGE.

VEL is unchanged (long EMA(5000) anchor + MR_STRENGTH=10 already wins
+$1.9K vs the range-position version). VEVs unchanged.
"""
