...
class BotEventParams:
    ...
    M14_WEIGHT = 0.1
    # Mark 55 (VEL aggressor cycler) marker. Mark 55 buys VELVET → next-window
    # mid +0.076 across n=598 events, qty=3254 (2x M14's volume). Mark 55 sells
    # symmetrically. Sign: bullish on M55 buy, bearish on M55 sell.
    M55_WEIGHT = 0.0
    # Cross-product: VEV_4000 Mark 14 → Mark 38 trades. Per analyze_bot_extras2,
    # VEL observable -0.25 across all 3 days (D1 -0.21 / D2 -0.32 / D3 -0.21),
    # 232 events / 458 qty. Mark 38 buying synthetic-VEL VEV_4000 = bagholder
    # signal → SHORT VEL. Sign: bearish on Mark 38 buys VEV_4000.
    VEV4000_M38_WEIGHT = 0.0
...
def _ingest_velvet_event_aggressive(market_trades, order_depths,
    ...
    Also ingests VEV_4000 Mark 38 buys as bearish VEL bias (cross-product)."""
    if not market_trades:
        return
    # VEV_4000 cross-product: Mark 14 -> Mark 38 trades signal VEL bearish.
    v4k_w = BotEventParams.VEV4000_M38_WEIGHT
    if v4k_w > 0:
        v4k_trades = market_trades.get("VEV_4000")
        if v4k_trades:
            for t in v4k_trades:
                if (getattr(t, "buyer", None) == "Mark 38"
                        and getattr(t, "seller", None) == "Mark 14"):
                    velvet_event_box[0] -= getattr(t, "quantity", 0) * v4k_w
    ...
    for t in velvet_trades:
        ...
        if m14_w > 0:
            if buyer == "Mark 14":
                ...
                velvet_event_box[0] -= qty * wt * m14_w
        # Mark 55 (experimental). Same sign as M14 (passive-MM-style).
        m55_w = BotEventParams.M55_WEIGHT
        if m55_w > 0:
            if buyer == "Mark 55":
                velvet_event_box[0] += qty * wt * m55_w
            elif seller == "Mark 55":
                velvet_event_box[0] -= qty * wt * m55_w
