...
class _VevBase:
    ...
    OWN_MR = 0
    BASE_QTY = 15
    # Round-2 OSMIUM-style execution overlay (off by default; per-strike below).
    # Crosses past fv_eff by URGENCY_EDGE*ramp when |target-pos|/POS_LIMIT
    # exceeds URGENCY_THRESHOLD, and adds NARROW_EDGE when spread<=NARROW_SPREAD
    # and book is imbalanced. Adapted from round 2 final (sub 352410) which
    # tuned URGENCY_EDGE=3, URGENCY_THRESHOLD=0.5, NARROW_EDGE=1 on OSMIUM.
    URGENCY_EDGE = 0
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        if velvet_mid is not None:
            for P in VOUCHER_PARAMS:
                ...
                result[P.SYMBOL] = self._exec_voucher(state, P, fair, target)
    ...
    def _exec_voucher(self, state: TradingState, P, fv: float,
                      target_pos: int) -> List[Order]:
        """Round-2 OSMIUM-style execution: urgency-edge + narrow-spread edge
        on top of the standard cross-and-quote path. fv is the delta-regressed
        fair (alpha + DELTA*velvet_mid)."""
        od = state.order_depths[P.SYMBOL]
        pos = state.position.get(P.SYMBOL, 0)
        if not od.buy_orders or not od.sell_orders:
            return []
        ...
        # Urgency edge: ramps when |target-pos|/POS_LIMIT > URGENCY_THRESHOLD
        ...
        if mag > P.URGENCY_THRESHOLD:
            ...
            ramp = 0.0
        buy_edge = round(P.URGENCY_EDGE * ramp) if raw_urg > 0 else 0
        sell_edge = round(P.URGENCY_EDGE * ramp) if raw_urg < 0 else 0

        # Narrow-spread book-imbalance edge
        spread = best_ask - best_bid
        if spread <= P.NARROW_SPREAD:
        ...
        # Passive penny-inside, clipped vs fv_eff
        base_bid = best_bid + 1
        base_ask = best_ask - 1
