"""Imbalance-tiered MA strategy.

Three confidence tiers for TOMATOES taking:

  1. CONFIRMED (imbalance nonzero, ~7% of ticks):
     Imbalance confirms direction. Take with full size, no extra edge needed.
     - imbalance < 0 (tight ask) → BUY if ask < fv
     - imbalance > 0 (tight bid) → SELL if bid > fv

  2. NEUTRAL (imbalance == 0, ~93% of ticks):
     No directional signal. Still allow taking, but require a larger edge
     (NEUTRAL_EDGE) and cap the quantity (NEUTRAL_MAX_QTY) to limit damage
     from MA lag on these ambiguous ticks.

  3. GUARD (PnL drawdown triggered):
     Block taking on losing side, aggressively clear position.
...
class StockTrader(ProductTrader):
    """Imbalance-tiered MA strategy for TOMATOES."""

    # --- MA parameters ---
    FAST_WINDOW = 10
    SLOW_WINDOW = 200
    SIGNAL_MULT = 0.5      # Mean-reversion bias: FV sits above mid in downtrends.
                            # Safe because imbalance gate blocks wrong-direction takes.

    # --- Neutral-tick taking parameters ---
    NEUTRAL_EDGE = 2       # (Inactive: neutral ticks never have fills due to 13-14pt spread)
    NEUTRAL_MAX_QTY = 5

    # --- Risk parameters ---
    SPREAD = 3
    CLEAR_THRESHOLD = 40
    GAMMA = 0.0            # Imbalance gate handles direction; GAMMA is redundant and costs alpha
    ...
    def run(self, state: TradingState, price_history: List[float], trader_state: dict) -> List[Order]:
        ...
        # --- NORMAL MODE: tiered taking ---
        buy_capacity = self.pos_limit - position
        sell_capacity = self.pos_limit + position

        if imbalance < -0.001:
            # CONFIRMED: tight ask → mid will revert UP → BUY with full size
            if order_depth.sell_orders:
                for ask_price in sorted(order_depth.sell_orders.keys()):
                    if ask_price < fv and buy_capacity > 0:
                        ask_vol = -order_depth.sell_orders[ask_price]
            ...
            # CONFIRMED: tight bid → mid will revert DOWN → SELL with full size
            if order_depth.buy_orders:
                for bid_price in sorted(order_depth.buy_orders.keys(), reverse=True):
                    if bid_price > fv and sell_capacity > 0:
                        bid_vol = order_depth.buy_orders[bid_price]
            ...
            # NEUTRAL: no imbalance signal. Allow taking with tighter edge and capped size.
            if order_depth.sell_orders:
                for ask_price in sorted(order_depth.sell_orders.keys()):
                    if ask_price < fv - self.NEUTRAL_EDGE and buy_capacity > 0:
                        ask_vol = -order_depth.sell_orders[ask_price]
                        qty = min(ask_vol, buy_capacity, self.NEUTRAL_MAX_QTY)
                        orders.append(Order(self.name, ask_price, qty))
                        buy_capacity -= qty

            if order_depth.buy_orders:
                for bid_price in sorted(order_depth.buy_orders.keys(), reverse=True):
                    if bid_price > fv + self.NEUTRAL_EDGE and sell_capacity > 0:
                        bid_vol = order_depth.buy_orders[bid_price]
                        qty = min(bid_vol, sell_capacity, self.NEUTRAL_MAX_QTY)
                        orders.append(Order(self.name, bid_price, -qty))
                        sell_capacity -= qty
