"""Round 4 v3 — long-anchor variant on max_scale.

Builds on max_scale ($426K BT champion). Major change: ANCHOR_SPAN dramatically
increased so the slow EMA stays near FIXED_ANCHOR throughout the day instead
of drifting toward intra-day price. Combined with the existing FIXED_ANCHOR
prior, this means the algo MR-reverts toward the cross-day mean even on
heavy-drift days, capturing more reversion.

| param            | max_scale | v3        | reason |
|------------------|----------:|----------:|--------|
| HYD ANCHOR_SPAN  |     5,000 | 1,000,000 | HYD cross-day stable at 9990 per memory; trust prior |
| VEL ANCHOR_SPAN  |     5,000 |    30,000 | balance: trust prior more than 5K but adaptive vs 1M |
| VEL MR_STRENGTH  |        12 |        10 | longer anchor → bigger devs → smaller MR mult OK |
| HYD FIXED_ANCHOR |     9,990 |     9,995 | sweep finds optimum at 9995 (+$2K) |

BT: $478,502 (+$52,408 / +12.3% vs max_scale $426,094).
LOOCV picks longer-anchor unanimously across all hold-outs.

REGIME RISK (per feedback_bt_loocv_vs_platform.md): long-anchor algos bet
the cross-day mean is the equilibrium. If platform regime has a different
equilibrium, the algo bleeds. Sensitivity sweep on FIXED_ANCHOR shows VEL
optimum at 5250 with halved gains at ±5 (5245→+$48K, 5255→+$39K, 5260→
−$7K). HYD less sensitive (9985→+$54K, 9995→+$60K, 10000→+$60K). Use
MODERATE values not infinite. If a future submission regresses, dial
ANCHOR_SPAN back toward 10K-30K.

History (max_scale chassis):
  Voucher spillover MAX(legacy 3xDELTA, SCALE_GAIN x vel_rel x POS_LIMIT).
  Preserves ATM aggression (legacy wins for DELTA >= 0.4) AND uncaps OTM
  (new wins for DELTA < 0.4). +$9.3K LOOCV vs baseline.
"""
...
class HydrogelParams:
    ...
    POS_LIMIT = 200
    INV_SKEW = 15
    # ANCHOR_SPAN reverted to 5000 (default adaptive). With PRIOR_BLEND=0.7
    # below, the effective anchor is 70% × FIXED_ANCHOR + 30% × adaptive_EMA.
    # On stable-regime BT days: EMA ≈ FIXED_ANCHOR so behavior matches v4.
    # On regime-shifted platform fragments (sub 530791 had local mean 10033):
    # EMA migrates toward local mean; effective anchor pulls 30% of that
    # adjustment, reducing the wrong-direction conviction the v4 long-anchor
    # locked in.
    ANCHOR_SPAN = 5000
    MR_STRENGTH = 5
    # FIXED_ANCHOR sweep with new long ANCHOR_SPAN: 9995 wins (+$60K vs 9990
    # +$57K). Memory's 9990 was correct; 9995 is a 5-tick refinement.
    FIXED_ANCHOR = 9995
    # PRIOR_BLEND mixes adaptive EMA with FIXED_ANCHOR. With ANCHOR_SPAN=1M,
    # EMA ≈ FIXED_ANCHOR anyway, so PRIOR_BLEND only matters when ANCHOR_SPAN
    # is shorter (allows EMA to drift). Set 0.7 to lean on prior, 0.3 lets
    # EMA respond to genuine regime shifts. v5 uses 0.7 + ANCHOR_SPAN=5000:
    # equivalent BT to v4 long-anchor but adapts on regime-shifted fragments.
    PRIOR_BLEND = 0.7
    # MM-ladder for spread harvesting on neutral-target ticks. HYD spread=16
    # in 92.5% of snapshots; existing logic captures only 1 tick (best±1).
    # Ladder posts at best_bid+3, best_bid+6 (and ask side mirror) to harvest
    # the 14 ticks of unused spread when |target| < threshold (algo isn't
    # loading directional position). +$188 BT (v3) when combined with PRIOR_BLEND.
    MM_LADDER_MIN_SPREAD = 8
    MM_LADDER_OFFSETS = (4, 8)
    MM_LADDER_QTY = 10
    MM_LADDER_NEUTRAL_THRESH = 30
    # Break-even-blend: anchor migrates toward avg_entry as |pos| grows.
    # 0 = pure FIXED_ANCHOR. 1 = pure avg_entry at full POS_LIMIT.
    # Helps the algo flatten profitably when forced into inventory by
    # asymmetric flow (e.g. Mark 38 buying-only on sub 530791).
...
class VelvetParams:
    ...
    POS_LIMIT = 200
    INV_SKEW = 3
    # ANCHOR_SPAN bumped from 5000 → 30000. Sweep peaks at 50K (+$57K) but 30K
    # is the more-adaptive choice — on the platform fragment (100K ticks), VEL_A=
    # 30K lets EMA migrate ~70% of the way to current price (still slow but
    # not glacial). Trades 5K of BT gain for fragment-regime safety.
    ANCHOR_SPAN = 30000
    # MR_STRENGTH = 10 (down from 12). Longer anchor produces larger |dev|
    # because anchor doesn't follow drift. Smaller MR mult achieves equivalent
    # target sizing. Sweep peak at 10 with new chassis.
    MR_STRENGTH = 10
    ...
    DRIFT_SPAN = 1000
    DRIFT_GAIN = 0.0
    # Voucher-implied VEL signal: weighted avg of (vev_mid - alpha) / DELTA
    # over ITM strikes (DELTA>0.3). When implied_S diverges from velvet_mid,
    # bias VEL target. 8-config sweep: GAIN=3.0 wins per-day positive on
    # all 3 days (D1 +3.1K / D2 +8.1K / D3 +2.5K, total +13.8K BT).
    # LOOCV-strict picks gain=20 on D2 (overfits) but gain=3.0 is the
    # robust uniform-positive choice.
    IMPLIED_S_GAIN = 3.0
    IMPLIED_S_CAP = 50
...
class _VevBase:
    ...
    MIN_SAMPLES = 100
    AGG_OVERSHOOT = 0
    # Cross-strike stat-arb: each tick, compare each strike's residual
    # deviation from its own alpha against the mean over all 8 strikes.
    # Anomaly = locally rich/cheap; fade via target bias.
    # XSTRIKE_ARB_GAIN=0 disables. Tolerance filters small noise; cap bounds
    # the contribution so the directional alpha isn't drowned.
    XSTRIKE_ARB_GAIN = 0.0
    XSTRIKE_ARB_THRESHOLD = 0.0
    XSTRIKE_ARB_CAP = 50
...
class Trader:
    ...
    def run(self, state: TradingState):
        ...
        # Voucher-implied VEL signal: each strike's vev_mid implies a VEL value
        # via inverse-DELTA. Weighted average across strikes → independent S
        # estimate. If diverges from actual velvet_mid, VEL is mispriced
        # relative to its options chain → bias VEL target.
        # Computed before voucher loop so it can also feed VEL re-pricing.
        if velvet_mid is not None and getattr(VelvetParams, "IMPLIED_S_GAIN", 0.0) != 0:
            implied_S_sum = 0.0; weight_sum = 0.0
            for P_v in VOUCHER_PARAMS:
                if P_v.SYMBOL not in state.order_depths:
                    continue
                od_imp = state.order_depths[P_v.SYMBOL]
                vmid = _micro_mid(od_imp)
                if vmid is None or P_v.DELTA < 0.3:
                    continue  # OTM strikes too noisy for inverse-DELTA
                alpha_imp = ema.get(P_v.SYMBOL, 0.0)
                # implied_S = (vev_mid - alpha) / DELTA   (so DELTA*S + alpha = vev_mid)
                imp_S = (vmid - alpha_imp) / P_v.DELTA
                w = P_v.DELTA  # weight by DELTA (more reliable for ITM)
                implied_S_sum += imp_S * w
                weight_sum += w
            if weight_sum > 0:
                implied_S = implied_S_sum / weight_sum
                divergence = implied_S - velvet_mid
                # Bias VEL target by gain × divergence (cap at SIGNAL_CAP).
                imp_gain = getattr(VelvetParams, "IMPLIED_S_GAIN", 0.0)
                imp_cap = getattr(VelvetParams, "IMPLIED_S_CAP", 50)
                imp_bias = max(-imp_cap, min(imp_cap, int(round(imp_gain * divergence))))
                # Add to VEL target — re-exec orders below
                if imp_bias != 0 and VelvetParams.SYMBOL in state.order_depths:
                    new_vel_target = max(-VelvetParams.POS_LIMIT,
                                         min(VelvetParams.POS_LIMIT,
                                             int(round(velvet_target_rel * VelvetParams.POS_LIMIT)) + imp_bias))
                    velvet_target_rel = new_vel_target / VelvetParams.POS_LIMIT
                    result[VelvetParams.SYMBOL] = self._exec(state, VelvetParams, velvet_mid, new_vel_target)

        if velvet_mid is not None:
            # Pass 1: compute current residuals + alphas for cross-strike arb
            voucher_state = []  # list of (P, od_v, vev_mid, residual_anom, fair)
            for P in VOUCHER_PARAMS:
                ...
                # Per-strike instantaneous residual deviation from its own alpha
                resid_dev = residual - cur_alpha
                voucher_state.append((P, od_v, vev_mid, resid_dev, fair))

            # Cross-strike anomaly = resid_dev_K − mean_over_strikes(resid_dev)
            # If positive: strike K is locally RICH → bias short. Negative: cheap → long.
            xstrike_gain = getattr(_VevBase, "XSTRIKE_ARB_GAIN", 0.0)
            mean_dev = 0.0
            if xstrike_gain != 0 and len(voucher_state) >= 3:
                mean_dev = sum(v[3] for v in voucher_state) / len(voucher_state)

            # Pass 2: build target with cross-strike bias overlay
            for P, od_v, vev_mid, resid_dev, fair in voucher_state:
                ...
                # Cross-strike arb: fade local anomaly. Tolerance via THRESHOLD;
                # cap contribution at XSTRIKE_CAP to avoid overshoot.
                xs_target = 0
                if xstrike_gain != 0:
                    anomaly = resid_dev - mean_dev
                    threshold = getattr(_VevBase, "XSTRIKE_ARB_THRESHOLD", 0.0)
                    cap = getattr(_VevBase, "XSTRIKE_ARB_CAP", 50)
                    if abs(anomaly) > threshold:
                        sign_excess = (anomaly - threshold) if anomaly > 0 else (anomaly + threshold)
                        xs_target = int(round(-xstrike_gain * sign_excess))
                        xs_target = max(-cap, min(cap, xs_target))
                target = int(round(spillover + own_mr + xs_target))
    ...
    def _mr(self, state: TradingState, P, fv: float,
        ...
        if P.ANCHOR_SPAN > 0:
            ...
            ema[P.SYMBOL] = L
            anchor = L
            # PRIOR_BLEND: Bayesian shrinkage toward FIXED_ANCHOR. On regime-
            # shifted platform fragments where local mean differs from prior,
            # 100% prior trust (long ANCHOR_SPAN) is brittle (sub 530791 HYD
            # made only $1,235 on a fragment with mean=10033 vs FIXED=9990).
            # PRIOR_BLEND>0 mixes adaptive EMA with the prior; effectively
            # reduces conviction when the chassis's anchor estimate is wrong.
            ...
            # Break-even-blend: as |pos|/POS_LIMIT grows, blend the anchor toward
            # our avg_entry price. The intuition: when forced into a heavy short
            # at avg 10024 (e.g. sub 530791) while FIXED_ANCHOR=9995 says we
            # should be even more short, the algo never tries to flatten at a
            # profitable exit. By migrating the anchor toward our actual fill
            # VWAP as |pos| grows, target → flat when mid hits avg_entry — a
            # natural break-even unwind. weight = BREAK_EVEN_BLEND × |pos|/LIMIT.
            be_blend = getattr(P, "BREAK_EVEN_BLEND", 0.0)
            if be_blend > 0:
                cur_pos_be = state.position.get(P.SYMBOL, 0)
                ae = avg_entry.get(P.SYMBOL, 0.0)
                if cur_pos_be != 0 and ae > 0:
                    pos_frac = min(1.0, abs(cur_pos_be) / P.POS_LIMIT)
        ...
        # Hard stop-loss: when |pos| * (avg_entry - mid)*sign(pos) < threshold,
        # force-unwind toward 0. PROFIT_DECAY handles winning positions; this
        # handles bleeding losers that MR keeps holding waiting for reversion.
        stop_loss = getattr(P, "STOP_LOSS_TICKS", 0)
        if stop_loss > 0 and pos != 0:
            ae = avg_entry.get(P.SYMBOL, 0.0)
            if ae > 0:
                pnl_per_unit = (fv - ae) if pos > 0 else (ae - fv)
                if pnl_per_unit < -stop_loss:
                    # Force target toward 0 (proportionally to loss magnitude)
                    target = int(round(target * 0.3))
    ...
    def _do_orders(self, P, fv: float, pos: int, target_pos: int,
        ...
        # MM-ladder overlay: post passive quotes inside the spread at multiple
        # levels to harvest oscillation MM on wide-spread products. Only fires
        # when |target_pos| <= MM_LADDER_NEUTRAL_THRESH so it doesn't conflict
        # with strong directional MR positioning. HYD has spread=16 in 92.5%
        # of snapshots — there are 14 ticks of unharvested spread per round
        # trip when target is neutral.
