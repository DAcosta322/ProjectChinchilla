...
Copy of 0418b + max-volume wall floor on resting quotes (pe049395 idea).
When posting passive quotes, clamp to stay IN FRONT of the dominant
(highest-volume) bid/ask level. Rationale: fills behind the MM wall are
likely adverse (they happen only when someone aggressively sweeps past
the wall, which means they have info). Pure post-clamp — no conflict
with DRIFT/INV_SKEW.

Set ENABLE_WALL_FLOOR = False to disable.
"""
...
class OsmiumParams:
    ...
    ENABLE_WALL_FLOOR = True  # clamp rests to >= max-vol bid+1 / <= max-vol ask-1
    WALL_MIN_VOL = 10         # only treat a level as a "wall" if vol >= this
...
class Trader:
    ...
    def _trade_osmium(self, state: TradingState, prices: List[float]) -> List[Order]:
        ...
        # Max-volume wall floor (pe049395): never rest behind the dominant
        # level. Fills behind the wall require aggressive sweepers, which
        # tend to be informed flow. Only treat levels with vol >= WALL_MIN_VOL
        # as "walls" — ignore 1-lot spoofs.
        if P.ENABLE_WALL_FLOOR:
            wall_bids = [p for p, v in od.buy_orders.items() if v >= P.WALL_MIN_VOL]
            wall_asks = [p for p, v in od.sell_orders.items() if -v >= P.WALL_MIN_VOL]
            if wall_bids:
                maxamt_bid = max(wall_bids, key=lambda p: od.buy_orders[p])
                our_bid = max(our_bid, maxamt_bid + 1)
            if wall_asks:
                maxamt_ask = min(wall_asks, key=lambda p: od.sell_orders[p])
                our_ask = min(our_ask, maxamt_ask - 1)
            # Re-enforce: our_bid must stay < our_ask and below fv_eff
            if our_bid >= our_ask:
                our_bid = min(our_bid, our_ask - 1)
            our_bid = min(our_bid, fv_eff - 1)
            our_ask = max(our_ask, fv_eff)
