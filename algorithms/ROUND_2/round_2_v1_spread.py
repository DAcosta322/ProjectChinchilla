"""Strategy 1: Skip resting quotes on tight-spread ticks.

Identical to round_2.py except: when spread < MIN_POST_SPREAD, Phase 2 is
skipped entirely — we only take mispriced orders, never post passive quotes.
Target: eliminate negative-edge fills from penny-up rests at near-mid prices.
"""
...
class OsmiumParams:
    ...
    INV_SKEW = 2
    DRIFT_STRENGTH = 25
    MIN_POST_SPREAD = 3        # NEW: skip resting when spread < this
...
class Trader:
    ...
    def _trade_osmium(self, state: TradingState, prices: List[float]) -> List[Order]:
        ...
        # Strategy 1: skip Phase 2 entirely when spread is tight
        if spread < P.MIN_POST_SPREAD:
            return orders
