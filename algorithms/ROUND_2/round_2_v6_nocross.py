"""Strategy 6: Never-cross-mid resting clamp.

Analysis of live log 288855 shows:
- 16 sell fills below mid, qty 91, total edge lost -$366
- 8 buy fills above mid, qty 50, total edge lost -$168
Root cause: when DRIFT pushes target far from pos, INV_SKEW drags fv_eff
across mid, so our clamp (best_ask-1 or fv_eff) ends up posting below mid.

Fix: our_ask >= mid + MID_MARGIN, our_bid <= mid - MID_MARGIN.
MID_MARGIN=0: never cross mid exactly.
MID_MARGIN=1: require one tick of edge from mid.
"""
...
class OsmiumParams:
    ...
    MID_MARGIN = 0            # NEW: min edge from mid for resting orders
...
class Trader:
    ...
    def _trade_osmium(self, state: TradingState, prices: List[float]) -> List[Order]:
        ...
        # Inventory-skewed clamp
        our_bid = min(our_bid, fv_eff - 1)
        our_ask = max(our_ask, fv_eff)

        # NEW: never cross mid. Use plain mid (not microprice) for this clamp
        # so the "don't post below mid" line doesn't drift with thin-side skew.
        if best_bid is not None and best_ask is not None:
            plain_mid = (best_bid + best_ask) / 2
            # our_ask must sit at/above plain_mid + MID_MARGIN
            min_ask = plain_mid + P.MID_MARGIN
            # use ceil-ish: round-up to nearest integer price
            import math
            our_ask = max(our_ask, math.ceil(min_ask))
            # our_bid must sit at/below plain_mid - MID_MARGIN
            max_bid = plain_mid - P.MID_MARGIN
            our_bid = min(our_bid, math.floor(max_bid))
