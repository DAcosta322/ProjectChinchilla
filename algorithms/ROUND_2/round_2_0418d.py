...
Copy of 0418b + adverse-volume filter (Carter/Eric inspired).
When taking against the book, skip any level with volume > ADVERSE_VOLUME
(likely informed flow — bot knows something). Targets the ~$500 of
negative-edge fills observed in live 288855 where our own rests got
picked off by size-weighted adverse flow.

Doesn't alter DRIFT/INV_SKEW architecture — purely additive guard on Phase 1.
"""
...
class OsmiumParams:
    ...
    ADVERSE_VOLUME = 20       # skip taking levels with |qty| > this (informed flow)
        ...
        if od.sell_orders:
            for price in sorted(od.sell_orders.keys()):
                if price < fv_eff + buy_edge and buy_cap > 0:
                    level_size = -od.sell_orders[price]
                    if level_size > P.ADVERSE_VOLUME:
                        continue  # informed flow guard
        ...
        if od.buy_orders:
            for price in sorted(od.buy_orders.keys(), reverse=True):
                if price > fv_eff - sell_edge and sell_cap > 0:
                    level_size = od.buy_orders[price]
                    if level_size > P.ADVERSE_VOLUME:
                        continue
