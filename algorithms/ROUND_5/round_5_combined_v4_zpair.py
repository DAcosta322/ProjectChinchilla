...
class SP:
    ...
    # Per-pair Z_FULL (override). 0.0 = use Z_FULL_ENTRY/EXIT global.
    Z_FULL_A = 0.0
    Z_FULL_B = 0.0
    ...
    def run(self, state: TradingState):
        ...
        if all(p in sp_mids for p in SNACK_PAIRS):
            ...
            if sp_n >= SP.WARMUP_TICKS:
                ...
                z_full_A_val = SP.Z_FULL_A if SP.Z_FULL_A > 0 else SP.Z_FULL_ENTRY
                z_full_B_val = SP.Z_FULL_B if SP.Z_FULL_B > 0 else SP.Z_FULL_ENTRY
                scale_entry_A = POS_LIMIT / z_full_A_val
                scale_entry_B = POS_LIMIT / z_full_B_val
                scale_exit  = POS_LIMIT / SP.Z_FULL_EXIT

                def directional_target(z, pair_pos_sign, scale_entry):
                    raw_unsigned = -z
                ...
                tgt_choc  = directional_target(z_A,  1 if pos_choc  > 0 else (-1 if pos_choc  < 0 else 0), scale_entry_A)
                tgt_van   = directional_target(-z_A, 1 if pos_van   > 0 else (-1 if pos_van   < 0 else 0), scale_entry_A)
                tgt_straw = directional_target(z_B,  1 if pos_straw > 0 else (-1 if pos_straw < 0 else 0), scale_entry_B)
                tgt_rasp  = directional_target(-z_B, 1 if pos_rasp  > 0 else (-1 if pos_rasp  < 0 else 0), scale_entry_B)
