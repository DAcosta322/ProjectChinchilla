"""Round 5 combined v4f — production submission target.

Integrates the OTHER_8 stability variant (v4f_own_anchor) with PEBBLES
(combined_v3 / baseline_l: dev_XL phantom-pos) and SNACKPACK (v5e). PEBBLES
and SNACKPACK sub-strategies are byte-equivalent to combined_v3.py — only
the OTHER_8 block carries the v4f changes:

  - cool-down (5 ticks) on the side that just filled
  - chronic-loser skip-list: SLEEP_POD_LAMB_WOOL, ROBOT_DISHES
  - own-fill-price anchor at pos==0 with 5000-ts expiry

BT $362,793.50 across days 2/3/4 (was $339K under combined_v3).
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        last_own_buy   = data.get("last_own_buy", {})
        last_own_sell  = data.get("last_own_sell", {})
        last_buy_ts    = data.get("last_buy_ts", {})
        ...
        # ====================================================================
        # 1. PEBBLES — UNCHANGED (combined_v3 / baseline_l: dev_XL phantom-pos)
        # ====================================================================
        peb_books = {}
        peb_mids = {}
        for p in PEBBLES:
        ...
        if len(peb_books) == 5:
            ...
            # arb_sell branch removed — analyze_pebbles_signal.py shows
            # max(sum_bb - 50000) = -1 across all 3 BT days; trigger never fires.
            # Variant-A BT confirmed identical PnL ($362,793.50 exact) with the
            # branch deleted, so this is pure dead-code removal.
        ...
        # ====================================================================
        # 2. OTHER 8 — v4f anchor + cool-down + skip chronic losers
        # ====================================================================
        other_books = {}
        for p in OTHER_8:
            if p in SKIP_MM:
