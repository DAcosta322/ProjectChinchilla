"""SNACKPACK v4: v3 + hard-coded prior std (fixes warmup hair-trigger).

v3 had MIN_STD=20 floor. Steady-state EMA-residual std on the BT data is
actually ~225 for factor A (CHOC-VAN) and ~270 for factor B (STRAW-RASP).
With std floored at 20, the first ~3000 ticks of each day produce z-scores
5-10x too large, causing instant +/- POS_LIMIT flips on small spread moves.

v4 changes:
  - Per-factor MIN_STD: A=150, B=200 (~10th-percentile of empirical std).
  - Initialize variance to (MIN_STD)^2 so std starts realistic, not 0.
  - Z_AGG=2.5 (was 99 / passive-only): with realistic sigma, |z|>=2.5
    means a ~500-tick spread move and is worth crossing for. Pure passive
    leaves edge on the table at the few large dislocations per day.

BT result on days 2/3/4 (sweep winner):
    HL_MEAN=4000, Z_FULL=0.2, Z_AGG=2.5
    d2=55,540  d3=32,388  d4=27,913   total = $115,841 (+$6,801 vs v3)

Empirical steady-state std (data days 2/3/4):
  A:  final=223/233/270  p50_post5k=223/224/276  p10_post5k=161/150/247
  B:  final=279/287/272  p50_post5k=255/285/259  p10_post5k=211/273/241
"""
...
class P:
    ...
    Z_FULL = 0.2
    Z_AGG  = 2.5
    # Per-factor std floor (~10th-pct of empirical steady-state std)
...
class Trader:
    def run(self, state: TradingState):
        ...
        for k, v in factors.items():
            if k not in ema:
                ema[k] = v
                # Initialize variance to (MIN_STD)^2 so std is realistic from tick 1
                var[k] = P.MIN_STD[k] ** 2
            else:
