"""SNACKPACK v5d: v4 + 4-product basket sum-arb (CHOC+VAN+STRAW+RASP).

Pattern analysis showed CHOC+VAN+STRAW+RASP sum has std=80-170 across
days (vs sum of 5 = 124-161). On day 3 the 4-sum std is 81 — tight enough
to support sum-arb on top of the per-pair MR.

Strategy: same per-pair factor MR as v4. Additionally, when book quotes
make the basket bid-sum > rolling mean + threshold, sell all 4. When
ask-sum < rolling mean - threshold, buy all 4.
"""
...
class P:
    ...
    Z_FULL = 0.2
    Z_AGG  = 2.5
    MIN_STD = {"A": 150.0, "B": 200.0, "SUM4": 100.0}
    WARMUP_TICKS = 200
    MIN_GAP = 1
    BASKET_Z_THRESH = 2.0
    BASKET_QTY = 3
...
class Trader:
    def run(self, state):
        ...
        sum4 = mids[CHOC] + mids[VAN] + mids[STRAW] + mids[RASP]
        factors = {"A": f_A, "B": f_B, "SUM4": sum4}
        ...
        # ---- 4-product basket sum-arb ----
        if abs(z_SUM4) >= P.BASKET_Z_THRESH:
            if z_SUM4 > 0:  # all 4 collectively rich -> sell all 4
                qty = P.BASKET_QTY
                for p in TRADED:
