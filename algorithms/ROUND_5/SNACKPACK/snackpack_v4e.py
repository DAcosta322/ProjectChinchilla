"""SNACKPACK v4e: v4d + fast EMA for PISTA factor.

v4d issue: HL_MEAN=4000 for the PISTA factor f_P = PISTA - 0.29*(S-R)
caused the EMA to lag PISTA's directional drift. PISTA drifts down
heavily within day (-489/-124/-282), so f_P drifts down too, residual
stays negative all day, target stays LONG PISTA, bleeds.

v4e fix: use much shorter HL for PISTA factor (HL_MEAN_P = 500 ticks).
The EMA catches drift quickly so residual MR signal is meaningful.

A and B keep HL_MEAN=4000 (those work fine — anti-pair sums are stable).
"""
...
class P:
    POS_LIMIT = 10
    # Per-factor EMA halflives
    HL_MEAN = {"A": 4000, "B": 4000, "P": 500}
    HL_VAR  = {"A": 8000, "B": 8000, "P": 1000}
    Z_FULL = 0.2
    Z_AGG  = 2.5
...
ALPHA_M = {k: 1.0 - 0.5 ** (1.0 / hl) for k, hl in P.HL_MEAN.items()}
ALPHA_V = {k: 1.0 - 0.5 ** (1.0 / hl) for k, hl in P.HL_VAR.items()}
    ...
    def run(self, state: TradingState):
        ...
        for k, v in factors.items():
            if k not in ema:
                ...
                ema[k] = ema[k] + ALPHA_M[k] * (v - ema[k])
                var[k] = (1.0 - ALPHA_V[k]) * var[k] + ALPHA_V[k] * (v - ema[k]) ** 2
        n += 1
