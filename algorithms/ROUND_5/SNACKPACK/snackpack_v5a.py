"""SNACKPACK v5a: v4 + book-imbalance execution filter.

Pattern analysis: L1 imbalance (bv-av)/(bv+av) correlates ~+0.10 with
next-tick return for every product. Use this as an entry filter:
when about to cross aggressively for factor MR, only cross if book
imbalance agrees with the desired direction (i.e., adverse selection
guard). When buying, require imb >= -0.1 (not strongly negative).
When selling, require imb <= +0.1.

Also tilts passive quoting: when imb favours us, quote inside more.
"""
...
class P:
    ...
    WARMUP_TICKS = 200
    MIN_GAP = 1
    IMB_GUARD = 0.1   # when buying, require imb >= -IMB_GUARD; selling: imb <= +IMB_GUARD
...
class Trader:
    def run(self, state):
        ...
        orders = {p: [] for p in TRADED}
        buy_used = {p: 0 for p in TRADED}; sell_used = {p: 0 for p in TRADED}
        ...
        for p in TRADED:
            bb, ba, bv, av = books[p]
            imb = (bv - av) / max(bv + av, 1)
            pos = state.position.get(p,0); tgt = target[p]; gap = tgt - pos
            if abs(gap) < P.MIN_GAP: continue
            wants_to_cross = product_z[p] >= P.Z_AGG

            if gap > 0:
                # Buying: only cross if book imbalance is not strongly negative
                aggressive = wants_to_cross and (imb >= -P.IMB_GUARD)
                if aggressive:
                ...
                need = -gap
                # Selling: only cross if book imbalance is not strongly positive
                aggressive = wants_to_cross and (imb <= +P.IMB_GUARD)
                if aggressive:
