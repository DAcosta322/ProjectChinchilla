"""SNACKPACK v5b: v4 + PISTA book-imbalance momentum.

Pattern analysis: PISTA book imbalance (bv-av)/(bv+av) correlates ~+0.13
with next-tick PISTA return on all 3 days. 5-tick edge of +7 to +9 ticks
when |imb| > 0.3.

Strategy for PISTA: target = round(POS_LIMIT * imb_smoothed / IMB_FULL).
Smooth imbalance via short EMA (HL=20) to denoise. Position scales
linearly with smoothed imbalance.

Other 4 products: same as v4 (factor MR).
"""
...
class P:
    ...
    WARMUP_TICKS = 200
    MIN_GAP = 1
    # PISTA imbalance signal
    HL_IMB = 20            # smooth imbalance over 20 ticks
    IMB_FULL = 0.4         # |smoothed_imb| = 0.4 -> target = +/- POS_LIMIT
    IMB_AGG = 0.5          # cross when |smoothed_imb| > this
...
class Trader:
    def run(self, state):
        ...
        product_z = {CHOC:abs(z_A), VAN:abs(z_A), STRAW:abs(z_B), RASP:abs(z_B), PISTA:0.0}
        # PISTA aggressiveness: cross when |imb| > IMB_AGG
        pista_agg = abs(imb_pista) >= P.IMB_AGG
