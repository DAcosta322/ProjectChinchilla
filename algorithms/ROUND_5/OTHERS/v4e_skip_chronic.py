"""Stability variant E — v4c (cool-down) + skip-list for chronic losers.

Base: v4c_cooldown.py (PEBBLES dev-phantom + SNACKPACK v5e + cool-down on OTHER_8).
Change: skip MM entirely on a small set of structurally-broken products that
v4c BT shows lose every day:
  - SLEEP_POD_LAMB_WOOL: drift sign 1.00 (+++) per memory; persistent uptrend
    + pull-to-zero MM = bleed. v4c -$6,485 (3/3 days, 21/30 frags neg).
  - ROBOT_DISHES: narrow spread (7.35) + occasional monotone days. v4c -$5,361.

Parameters added: 0 (just the SKIP_MM set).
State added: 0 beyond v4c.
"""
