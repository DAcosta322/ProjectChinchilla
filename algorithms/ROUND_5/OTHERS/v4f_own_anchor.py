"""Stability variant F — v4e + own-trade-price anchors when pos=0 only.

Base: v4e_skip_chronic.py (cool-down + skip chronic losers).
Change: track each product's last own buy and last own sell price. Apply
"no guaranteed loss" anchors ONLY when pos == 0 (i.e., when we're choosing
opportunistically). When pos != 0 we still want to flatten freely, so the
anchor is ignored and existing pull-to-zero logic applies.

Anchor logic at pos == 0:
  - bid_post must be STRICTLY BELOW the last own sell price.
  - ask_post must be STRICTLY ABOVE the last own buy price.

Mechanism: after we sell at P, we know the market just paid P to take from us.
If we then bid above P with no inventory pressure, the next round trip is a
known loser. The anchor lets the book come back to us before re-engaging.
When inventory is non-zero, flattening still takes priority.

Parameters added: 0.
State added: per-product `last_own_buy` and `last_own_sell` ints in traderData.
"""
...
# Chronic losers identified from v4c BT — skip MM entirely on these products.
# See data/ROUND_5/v4c_losers/summary.txt for evidence.
SKIP_MM = {
    "SLEEP_POD_LAMB_WOOL",
    "ROBOT_DISHES",
}
...
class P:
    O8_COOLDOWN_TICKS = 5
    O8_ANCHOR_EXPIRY_TS = 5000   # sweep-winning sweet spot  # ticks (where 1 tick == 100 ts units)
...
class Trader:
    def run(self, state: TradingState):
        ...
        last_own_buy   = data.get("last_own_buy", {})    # {product: price}
        last_own_sell  = data.get("last_own_sell", {})
        last_buy_ts    = data.get("last_buy_ts", {})     # {product: ts of last buy}
        last_sell_ts   = data.get("last_sell_ts", {})

        ts_now = state.timestamp
        cool_units = P.O8_COOLDOWN_TICKS * 100

        # Process own_trades from prior tick: update cool-downs + anchor prices+ts.
        ...
        def pull_to_zero_mm_cd(p, books_map, mm_qty=10):
            """OTHER_8 stability variant — cool-down + own-trade-price anchors."""
            r = books_map.get(p)
            if r is None:
                return
            bb, ba, _, _ = r
            ...
            mm_bid = bb + 1 if bb + 1 < ba else None
            mm_ask = ba - 1 if ba - 1 > bb else None
            # Own-anchor: skip side that would lock losing RT, but only when
            # (a) inventory is flat (so we're not blocking flatten) and
            # (b) the anchor fill is recent (within ANCHOR_EXPIRY ts).
            if pos == 0:
                los = last_own_sell.get(p)
                lob = last_own_buy.get(p)
                los_age = ts_now - last_sell_ts.get(p, -10**18)
                lob_age = ts_now - last_buy_ts.get(p, -10**18)
                if mm_bid is not None and los is not None and los_age <= P.O8_ANCHOR_EXPIRY_TS \
                        and mm_bid >= los:
                    if los - 1 > bb:
                        mm_bid = los - 1
                    else:
                        mm_bid = None
                if mm_ask is not None and lob is not None and lob_age <= P.O8_ANCHOR_EXPIRY_TS \
                        and mm_ask <= lob:
                    if lob + 1 < ba:
                        mm_ask = lob + 1
                    else:
        ...
        # ---- OTHER 8 — cool-down MM, skip chronic losers ----
        other_books = {}
        for p in OTHER_8:
            if p in SKIP_MM:
                continue
        ...
        out_state = json.dumps({
            ...
            "last_own_buy":  last_own_buy,
            "last_own_sell": last_own_sell,
