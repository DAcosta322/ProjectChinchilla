"""Round 5 combined v3 — combined_v1 + XL dev-phantom-pos signal on PEBBLES.

Same structure as combined_v1 but PEBBLES section uses baseline_l logic:
  implied_XL = 50000 - sum(other 4 PEBBLES mids)
  dev_XL = mid_XL - implied_XL
  phantom_pos_XL = real_pos_XL + DEV_GAIN * dev_XL
  Use phantom_pos for one-sided MM gating on XL only.

DEV_GAIN=5 sweep-tuned on PEBBLES:
  - PEBBLES isolated BT: $35K → $49K (+$14K)
  - PEBBLES D4 frag 0: $765 → $3,735 (+$2,970)
  - All D4 fragments positive
  - Worst PEBBLES fragment: -$4K (D3 frag 6, others-driven not XL-driven)

SNACKPACK v5e and OTHER 40 unchanged from v1.
"""
...
class Trader:
    def run(self, state: TradingState):
        ...
        # ====================================================================
        # 1. PEBBLES — baseline_l: phantom-pos on XL via dev signal
        # ====================================================================
        peb_books = {}
        peb_mids = {}
        for p in PEBBLES:
        ...
        # ====================================================================
        # 2. OTHER 8 — pure pull-to-zero MM
        # ====================================================================
        other_books = {}
        for p in OTHER_8:
            r = _book(state.order_depths.get(p))
