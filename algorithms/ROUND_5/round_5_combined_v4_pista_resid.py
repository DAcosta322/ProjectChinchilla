...
class SP:
    ...
    # PISTA residual MR overlay: f_C = PISTA - PISTA_RESID_BETA*STRAW.
    # When |z_C| > Z_FULL_C, target_pista shifts by (-z_C * scale_C), capped.
    # Setting GAIN=0 disables; GAIN=1 = full snap to ±POS_LIMIT once |z_C|>Z_FULL_C.
    PISTA_RESID_BETA  = 0.59
    PISTA_RESID_GAIN  = 0.0   # 0 = disabled
    PISTA_RESID_ZFULL = 1.0   # |z_C| at which the residual contribution maxes
    MIN_STD_C         = 50.0  # reject extreme z when std collapses
...
class Trader:
    def run(self, state: TradingState):
        ...
        if all(p in sp_mids for p in SNACK_PAIRS):
            ...
            factors = {"A": f_A, "B": f_B}
            # Optional factor C: PISTA - beta*STRAW (only when PISTA available + enabled)
            have_C = SP.PISTA_RESID_GAIN > 0.0 and SNACK_PISTA in sp_mids
            if have_C:
                f_C = sp_mids[SNACK_PISTA] - SP.PISTA_RESID_BETA * sp_mids[SNACK_STRAW]
                factors["C"] = f_C
                min_std["C"] = SP.MIN_STD_C
            ...
            if sp_n >= SP.WARMUP_TICKS:
                ...
                if SNACK_PISTA in sp_books:
                    ...
                    if abs(z_B) >= SP.PISTA_LEV_THRESH and SP.PISTA_LEV != 1.0:
                        ...
                        tgt_pista = base_tgt
                    # Residual MR overlay: shift PISTA target by -z_C contribution
                    if SP.PISTA_RESID_GAIN > 0.0 and "C" in sp_var:
                        sd_C = max(SP.MIN_STD_C, sp_var["C"] ** 0.5)
                        z_C = (factors["C"] - sp_ema["C"]) / sd_C
                        # Each unit of z_C beyond ZFULL_C adds full POS_LIMIT (signed)
                        scale_C = POS_LIMIT / SP.PISTA_RESID_ZFULL
                        delta_pista = -z_C * scale_C * SP.PISTA_RESID_GAIN
                        tgt_pista = _clip(int(round(tgt_pista + delta_pista)),
                                           POS_LIMIT)
                    gap = tgt_pista - pos
                    if abs(gap) >= SP.MIN_GAP:
